function S2S_group_comparison_UMG(Sum_ses,Sum_norm,Sum_tot,stats_test,comparison,plot_specs)
...
%% Figure 1: Mean and STD of Normalized Values per Session (or subject), significance across sessions (or subjects)
...
plot_specs.title='Mean + STD of Normalized Values';
plot_specs.figure_name_suffix='comparison 1';
title_and_save(plot_specs);

%% Figure 2: Mean and STD of Normalized Differences (Left - right) per Session (or subject), significance across sessions (or subjects)
plot_specs.figure_handle=figure('units','normalized','outerposition',[0 0 1 1]);
for k=1:numel(FN_P)
    subplot(3,4,k)
    %xlim([0 15]);
...
plot_specs.title='Mean + STD of Normalized Differences (Left - right)';
plot_specs.figure_name_suffix='comparison 2';
title_and_save(plot_specs);

%% Figure 3: Number of significant Sessions (or subjects)
if numel(Sum_ses(1).Hit.SL) == numel(Sum_ses(2).Hit.SL)
    plot_specs.figure_handle=figure('units','normalized','outerposition',[0 0 1 1]);
    for k=1:numel(FN_P)
        subplot(3,4,k)
    ...
    plot_specs.title='Number of significant Sessions (or subjects)';
    plot_specs.figure_name_suffix='comparison 3';
    title_and_save(plot_specs);    
end

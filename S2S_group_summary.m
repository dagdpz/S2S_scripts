function [Sum_ses,Sum_norm,Sum_tot]=S2S_group_summary(M2S_output,subject_labels)
...
FN={'SL','SR','TL','TR','all'};
% Mean and STD of Normalized Values per Session (or subject), significance across sessions
...
for k=1:numel(fieldnames_perception_intention)
    ...
    for m=1:numel(P_fieldnames)
        for s=[1,-1]
            ...
            col_idx=col_idx+1;
                tot_subj_labels=subject_labels(idx_tot & ~idx_sig);
                sig_subj_labels=subject_labels(idx_sig);
            for t1=1:n_sig*s
